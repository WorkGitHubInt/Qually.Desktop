...
namespace QuallyFlash
{
    public class Account : BaseModel
    {
        ...
        {
            var document = Parser.ParseDocument(await Client.GetAsync("/member/account/?type=partage"));
            IHtmlCollection<IElement> names;
            IHtmlCollection<IElement> buttons;
            if (document.QuerySelector("#table-1") == null)
            {
                names = document.QuerySelectorAll("#table-0 .usergroup_2");
                buttons = document.QuerySelectorAll("#table-0 button");
            }
            else
            {
                names = document.QuerySelectorAll("#table-1 .usergroup_2");
                buttons = document.QuerySelectorAll("#table-1 button");
            }
            int i = 0;
            foreach (var item in names)
            {
                if (!buttons[i].ClassName.Contains("disabled"))
                {
                    if (item.TextContent == loginCo)
                    {
                        string id = item.GetAttribute("href").Split('=')[1];
        ...
        public async Task LoadCoAccounts()
        {
            ...
            if (document.QuerySelector("#table-1") == null)
            {
                var names = document.QuerySelectorAll("#table-0 .dark");
                var buttons = document.QuerySelectorAll("#table-0 button");
                if (names.Length == 0)
                {
                    throw new Exception();
                }
            ...
            {
                var names = document.QuerySelectorAll("#table-1 .dark");
                var buttons = document.QuerySelectorAll("#table-1 button");
                if (names.Length == 0)
                {
                    throw new Exception();
                }
        ...
        public async Task LoadFarms()
        {
            ...
            for (int i = 0; i < tabs.Length; i++)
            {
                ...
                {
                    Farms.Add(new Farm(tabs[i].TextContent.Trim(), "tab-all-horses", this));
                }
                else
                {
        ...
        public async Task Buy(Equipment equipment)
        {
            await Client.PostAsync("/marche/achat", $"id={equipment.Id}&mode=eleveur&nombre=1&typeRedirection=&idElement=");
        }
        ...
        public async Task StartTraining(ObservableCollection<Horse> horses)
        {
            ...
            {
                MessageBox.Show("Была выбрана выписка из кск, но не стоит свой резерв кск! Выберите необходимые настройки!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            LogSettings();
        ...
        public async Task<Horse> TrainHalf(Horse horse)
        {
            ...
            if (CheckAuto(document))
            {
                ...
                if (horse.Age > 120 && a[0].ClassName.Contains("disabled"))
                {
                    ...
                    Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                    {
                        MessageBox.Show(Application.Current.MainWindow, "Работа была остановлена! Лошадь находится в неверном периоде! Нельзя запускать лошадь, если ей осталось 4 месяца до родов (время, когда соревнования выключены)!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                    }));
                }
                if (Settings.SelfReserve && Settings.WriteOut && horse.Age >= 6 && horse.Age <= 96 && document.QuerySelector("#cheval-inscription") == null && document.QuerySelectorAll("#center-tab-main .button-text-3").Length == 0)
                {
                    ...
                    Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                    {
                        MessageBox.Show(Application.Current.MainWindow, "Работа была остановлена! Был поставлен свой резерв, но лошадь уже записана в какой-то кск!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                    }));
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            IoC.Kernel.Get<MainViewModel>().Horses[0] = MaleHorse;
                            IoC.Kernel.Get<MainViewModel>().Horses[1] = FemaleHorse;
                            IoC.Kernel.Get<MainViewModel>().SelectedSexHorse = FemaleHorse;
                        });
                    }
                }
                if (Settings.SchemeType == SchemeType.HalfPair)
                {
                    Application.Current.Dispatcher.Invoke(delegate
                    {
                        IoC.Kernel.Get<MainViewModel>().Horses[0] = horse;
                        IoC.Kernel.Get<MainViewModel>().SelectedSexHorse = horse;
                    });
                }
            }
            else
            {
                MessageBox.Show("Работа была остановлена! Не удалось выключить автотренировки/автопрогулки! Выключите вручную!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                Cts.Cancel();
            }
            return horse;
        }
        ...
        private async Task Training(Horse horse, IHtmlDocument document)
        {
            ...
            {
                if (horse.Age >= 36 && !horse.IsSpecialized)
                {
                ...
                if (Carrot.Amount < 100 && Settings.BuyCarrotMash)
                {
                    if (Equ < 21000)
                    {
                        ...
                        else
                        {
                            Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                            {
                                MessageBox.Show(Application.Current.MainWindow, $"На аккаунте {Login} закончилось экю и нет ресурсов для пополнения (или не выбраны для докупки)! Пополните запасы и нажмите ОК", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                            }));
                        }
                    }
        ...
        private async Task<Horse> BornCertain(IHtmlDocument document, Horse momHorse)
        {
            ...
            if (Settings.Heavy && heavyHorsing >= 3)
            {
                Cts.Cancel();
                if (Application.Current.Dispatcher.CheckAccess())
                {
                    MessageBox.Show(Application.Current.MainWindow, "Работа была остановлена! Исчерпан лимит случек для тяжеловозов!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
                else
                {
                    Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
