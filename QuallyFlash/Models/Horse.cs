...
namespace QuallyFlash
{
    public class Horse : BaseModel, ICloneable
    {
        ...
        public bool LoadInfo(IHtmlDocument document)
        {
            ...
            {
                string script = document.QuerySelectorAll("#page-contents script")[0].InnerHtml;
        ...
        public async Task Rename(IHtmlDocument document)
        {
            try
            {
                ...
                if (Sex == HorseSex.Female && !string.IsNullOrEmpty(acc.Settings.FemaleName))
                {
                    Name = $"{acc.Settings.FemaleName} {document.QuerySelector("#genetic-body-content").QuerySelectorAll("strong")[1].TextContent.Split(':')[1].Trim()}{nameSkill}";
                }
                else if (Sex == HorseSex.Male && !string.IsNullOrEmpty(acc.Settings.MaleName))
                {
                    Name = $"{acc.Settings.MaleName} {document.QuerySelector("#genetic-body-content").QuerySelectorAll("strong")[1].TextContent.Split(':')[1].Trim()}{nameSkill}";
                }
        ...
        public async Task<string> Action(IHtmlDocument document, string formId, string selector, string action)
        {
            ...
            if (!document.QuerySelector(selector).ClassName.Contains("action-disabled"))
            {
                switch (action)
                {
                    case "doGroom": Status = "чистка"; break;
                    case "doNight": Status = "сон"; break;
                    case "doAge": Status = "рост"; break;
                    case "doStroke": Status = "ласка"; break;
                    case "doDrink": Status = "пить"; break;
                    case "doEatTreat":
            ...
            else if (Age < 18)
            {
                ...
                if (hay != 0)
                {
                    ...
                    else if (acc.Hay.Amount < 100 && !acc.Settings.BuyFood)
                    {
                        Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                        {
                            MessageBox.Show(Application.Current.MainWindow, $"На аккаунте {acc.Login} закончился фураж и не стоит докупка! Пополните запасы и нажмите ОК", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                        }));
                    }
            ...
            else
            {
                ...
                if (hay != 0 || oat != 0)
                {
                    ...
                    } else if (acc.Oat.Amount < 100 && !acc.Settings.BuyFood)
                    {
                        Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                        {
                            MessageBox.Show(Application.Current.MainWindow, $"На аккаунте {acc.Login} закончился овес и не стоит докупка! Пополните запасы и нажмите ОК", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                        }));
                    }
        ...
        public async Task Train(IHtmlDocument document, Training training)
        {
            if (Age >= 24)
            {
                ...
                if (count > 0)
                {
                    Status = "тренировка " + training.Name;
            ...
            if (Age >= 36)
            {
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        {
                            Status = "Соревнование " + training.Name;
        ...
        private async Task CheckEquipment(Equipment equipment)
        {
            ...
            {
                if (acc.Equ <= equipment.Price)
                {
                    if (acc.Settings.MainProductToSell != ProductType.None && acc.MainProductToSell.Amount * acc.MainProductToSell.SellPrice > equipment.Price)
                    {
                        await acc.Sell(acc.MainProductToSell, (equipment.Price + 1000 / acc.MainProductToSell.SellPrice).ToString()).ConfigureAwait(false);
                    }
                    else if (acc.Settings.SubProductToSell != ProductType.None && acc.SubProductToSell.Amount * acc.SubProductToSell.SellPrice > equipment.Price)
                    {
                        await acc.Sell(acc.SubProductToSell, (equipment.Price + 1000 / acc.SubProductToSell.SellPrice).ToString()).ConfigureAwait(false);
                    } else
                    {
                        Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                        {
                            MessageBox.Show(Application.Current.MainWindow, $"На аккаунте закончилось экю и нет ресурсов для пополнения (или не выбраны для докупки)! Пополните запасы и нажмите ОК", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                        }));
                    }
                }
                await acc.Buy(equipment).ConfigureAwait(false);
        ...
        {
            Status = "хлыст";
            await acc.Client.PostAsync("/elevage/chevaux/doDonnerBonus", $"id={Id}&bonus=135").ConfigureAwait(false);
        }
        #endregion
        ...
        public async Task<Horse> Birth()
        {
            ...
            string sex = doc.GetElementsByClassName("align-top spacer-small-top spacer-right")[0].GetElementsByTagName("img")[0].GetAttribute("alt");
            ...
            string horseName = (sex == "male") ? acc.Settings.MaleName : acc.Settings.FemaleName;
