...
namespace QuallyFlash
{
    public class MainViewModel : BaseViewModel
    {
        ...
        public MainViewModel()
        {
            ...
            GlobalSettings = IoC.Kernel.Get<GlobalSettings>();
        ...
        private async void AddHorse()
        {
            if (SelectedHorse != null)
            {
                ...
                {
                    MessageBox.Show("Лошадь метрва! Добавление не возможно!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
            }
        }
        ...
        {
            if (!Account.IsWorking && Account.Type == AccountType.Normal)
            {
                logger.Info("Вход в соу");
                ((MainWindow)Application.Current.MainWindow).ShowPage(new LoginCoPage()
                {
                    DataContext = new LoginCoViewModel(Account)
                });
            }
        }
        ...
        private void OpenSettings()
        {
            ...
            {
                DataContext = new SettingsViewModel(Account.Settings.Clone() as Settings)
            });
        }
            ...
            if (!Account.IsWorking)
            {
                ...
                if ((Account.Settings.SchemeType == SchemeType.Pair && Horses.Count == 2) || (Account.Settings.SchemeType == SchemeType.HalfPair && Horses.Count == 1))
                {
                    ...
                    catch (OperationCanceledException)
                    {
                        ...
                        {
                            MessageBox.Show(Application.Current.MainWindow, "Отменено!", "", MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                        else
                        {
                    ...
                    catch (Exception e)
                    {
                        ...
                        if (Application.Current.Dispatcher.CheckAccess())
                        {
                            MessageBox.Show(Application.Current.MainWindow, $"Во время тренировок произошла ошибка! Все необходимые данные были записаны. Отправьте отчет об ошибке.", "", MessageBoxButton.OK, MessageBoxImage.Error);
                        }
                        else
                        {
                            Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                ...
                {
                    MessageBox.Show("Неверное кол-во лошадей под данную схему (пара/полупара)!", "", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
        ...
        private void OpenManager()
        {
            ...
            ((MainWindow)Application.Current.MainWindow).ShowPage(new ManagerPage()
            {
                DataContext = new ManagerViewModel(Account)
            });
        }
        ...
        private void SaveCoAccountToFile()
        {
            ...
            {
                MessageBox.Show("Аккаунт не соу!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }
    }
}
