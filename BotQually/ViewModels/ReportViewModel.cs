        ...
        private async void Send()
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    string postData = $"id={sha}&programId=1&type={Type}&body={Body}&version={typeof(MainWindow).Assembly.GetName().Version}&contacts={Contacts}&logmain={logmain}&logacc={logacc}&logerror={logerror}&status=Ожидает%20просмотра";
            ...
            {
                MessageBox.Show($"Произошла ошибка отправки отчета: {e.Message}!", "", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }
