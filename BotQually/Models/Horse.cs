...
namespace BotQually
{
    public class Horse
    {
        ...
        public async Task CentreReserve(Settings settings)
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    {
                        IoC.Kernel.Get<MainViewModel>().Notifications.Add($"Аккаунт: {acc.Login}; Нет резервных стойл.");
                    });
                    answer = "0";
                }
        ...
        #region Horsing
        public async Task<string> Birth(Settings settings, GlobalSettings globalSettings)
        {
            string maleName = settings.RandomNames ? globalSettings.MaleNames[new Random().Next(0, globalSettings.MaleNames.Length)] : settings.MaleName;
            string femaleName = settings.RandomNames ? globalSettings.FemaleNames[new Random().Next(0, globalSettings.FemaleNames.Length)] : settings.FemaleName;
            ...
            string sex;
            if (acc.Client.BaseAddress == "https://ar.howrse.com")
            {
                sex = doc.GetElementsByClassName("align-top spacer-small-top spacer-left")[0].GetElementsByTagName("img")[0].GetAttribute("alt");
            }
            else
            {
                sex = doc.GetElementsByClassName("align-top spacer-small-top spacer-right")[0].GetElementsByTagName("img")[0].GetAttribute("alt");
            }
            string postData;
            if (sex == "male")
            {
                postData = $"valider=ok&poulain-1={maleName}&affixe-1={affixValue}&elevage-1={farmValue}";
            }
            else
            {
                postData = $"valider=ok&poulain-1={femaleName}&affixe-1={affixValue}&elevage-1={farmValue}";
            }
            string answer;
            do
            {
        ...
        public async Task HorsingFemale(IHtmlDocument document, Settings settings)
        {
            ...
            catch (Exception e)
            {
                Application.Current.Dispatcher.Invoke(delegate
                {
                    IoC.Kernel.Get<MainViewModel>().Notifications.Add($"Ошибка ж. случки Аккаунт: {acc.Login}; Лошадь: {Name}");
                });
        ...
        public async Task<bool> CheckHorse(IHtmlDocument document, Settings settings)
        {
            ...
            else if (document.QuerySelector("#poulain-1") != null)
            {
                ...
                string sex = document.GetElementsByClassName("align-top spacer-small-top spacer-right")[0].GetElementsByTagName("img")[0].GetAttribute("alt");
                if (sex == "male")
                {
