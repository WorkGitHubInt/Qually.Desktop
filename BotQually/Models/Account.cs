...
namespace BotQually
{
    public class Account : BaseModel
    {
        ...
        public virtual async Task<bool> LogInCo(string loginCo, bool load)
        {
        ...
        public async Task StartParallel(GlobalSettings globalSettings)
        {
            try
            {
                ...
                foreach (var farm in Queue)
                {
                    ...
                    await farm.Run(Settings, globalSettings, Cts.Token).ConfigureAwait(false);
                }
            ...
            {
                MessageBox.Show($"Отменено \nАккаунт: {Login}\nЗавод: {ProgressFarm}\nЛошадь: {ProgressHorse}", "", MessageBoxButton.OK, MessageBoxImage.Information);
                Progress = "отменен";
                Client.Ct = new CancellationToken();
                logger.Info($"Аккаунт: {Login} остановлен параллельный прогон");
                Application.Current.Dispatcher.Invoke(delegate
                {
                    IoC.Kernel.Get<MainViewModel>().Notifications.Add($"Отмена Аккаунт: {Login}; Завод: {ProgressFarm}; Лошадь: {ProgressHorse}");
                });
            }
            catch (Exception e)
            {
                logger.Info($"Аккаунт: {Login} ошибка параллельный прогон");
                MessageBox.Show($"Во время прогона {Login} произошла ошибка! Завод: {ProgressFarm}; Лошадь: {ProgressHorse}. Все необходимые данные были записаны. Не забудьте отправить отчет.", "", MessageBoxButton.OK, MessageBoxImage.Error);
                ...
                Application.Current.Dispatcher.Invoke(delegate
                {
                    IoC.Kernel.Get<MainViewModel>().Notifications.Add($"Ошибка Аккаунт: {Login}; Завод: {ProgressFarm}; Лошадь: {ProgressHorse}");
                });
            }
            finally
            {
                ...
                Application.Current.Dispatcher.Invoke(delegate
                {
                    IoC.Kernel.Get<MainViewModel>().Notifications.Add($"Аккаунт: {Login} окончен; ОР добавлено: {OREND}");
                });
                IsRunning = false;
                EndActions();
            }
        ...
        public async Task StartOrder(CancellationToken ct, GlobalSettings globalSettings)
        {
            ...
            {
                ProgressFarm = farm.Name;
                await farm.Run(Settings, globalSettings, ct).ConfigureAwait(false);
            }
            IsDone = true;
            OREND = (OR.Amount - OrBegin).ToString();
            Application.Current.Dispatcher.Invoke(delegate
        ...
        {
            IoC.Kernel.Get<MainViewModel>().LoadCounts();
        }
